...
def train_ppo(**kwargs):
    ...
    train(
        n_updates=5000, print_every=1, eval_every=5, time_budget_sec=30 * 60, **kwargs
    )
...
def main():
    ...
    parser.add_argument(
        "--show-browser",
        action="store_true",
        help="PPO-only: show Chrome instead of headless mode",
    )
    parser.add_argument(
        "--browser-accelerate",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="PPO-only: override time budget",
    )
